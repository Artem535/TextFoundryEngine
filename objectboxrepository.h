...
namespace tf {
   ...
   *
   * @note Parameter schemas are stored separately in ObxBlockParam for queryability.
   */
  ...
  public:
     ...
     * If a block with the same ID and version already exists, it will be overwritten.
     */
   ...
   *
   * Provides persistent storage for Composition entities using ObjectBox database.
   * Stores compositions with their metadata, style profiles, and fragment references.
   *
   * Composition entities are stored across two tables:
   * - ObxComposition: composition metadata (id, version, state, styles, description)
  ...
  public:
     ...
     * its fragments. If a composition with the same ID and version already exists,
     * it will be overwritten (upsert behavior).
     */
     ...
     */
    [[nodiscard]] Result<Composition> load(const CompositionId &id, Version version) override;
     ...
     */
    [[nodiscard]] Error deprecate(const CompositionId &id, Version version) override;

  private:
    std::shared_ptr<obx::Store> store_; ///< ObjectBox store instance
    std::unique_ptr<obx::Box<ObxComposition> > box_composition_; ///< Composition entity box
    std::unique_ptr<obx::Box<ObxFragment> > box_fragment_; ///< Fragment entity box
  };
} // tf
