 ...
 *
 * Implements IBlockCache interface using IBlockRepository as the underlying
 * storage. Caches blocks in memory for fast repeated access during rendering.
...
class RepositoryBlockCache final : public IBlockCache {
 ...
 private:
  ...
  mutable std::unordered_map<std::pair<BlockId, Version>, Block,
                             BlockVersionHash>
      cache_;
