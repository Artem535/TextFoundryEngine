...
namespace tf {
BlockRepository::BlockRepository(std::shared_ptr<obx::Store> store)
...
Error BlockRepository::save(const Block& block) {
  ...
  TF_LOG_INFO("BlockRepository::save| Block saved: {} v{}.{}", block.Id(),
...
Result<Block> BlockRepository::LoadLatest(const BlockId& id) {
  ...
  if (version.HasError()) {
    TF_LOG_ERROR("BlockRepository::loadLatest| Error msg: {}",
...
std::vector<BlockId> BlockRepository::list(
  ...
  auto query =
      box_block_->query(ObxBlock_::type.equals(obx_block_type)).build();
...
Error BlockRepository::deprecate(const BlockId& id, Version version) {
  ...
  TF_LOG_INFO("BlockRepository::deprecate| Block deprecated, BlockId: {}", id);
  return Error{ErrorCode::Success};
}
...
CompositionRepository::CompositionRepository(std::shared_ptr<obx::Store> store)
    : store_(std::move(store)) {
...
Error CompositionRepository::save(const Composition& composition) {
  ...
  auto frag_query =
      box_fragment_->query(ObxFragment_::compositionId.equals(comp_id)).build();
  ...
  TF_LOG_INFO(
      "CompositionRepository::save| Composition saved, id: {}, version: {}.{}",
      composition.id(), composition.version().major,
      composition.version().minor);
  return Error{ErrorCode::Success, ""};
}

Result<Composition> CompositionRepository::load(const CompositionId& id,
                                                const Version version) {
  // Query composition by ID and version
  auto query =
      box_composition_
          ->query(ObxComposition_::compositionId.equals(id) &&
  ...
  if (!obx_comp) {
    TF_LOG_ERROR(
        "CompositionRepository::load| Composition not found: {} v{}.{}", id,
  ...
  auto frag_query =
      box_fragment_->query(ObxFragment_::compositionId.equals(obx_comp->id))
          .order(ObxFragment_::orderIndex)
          .build();
  ...
  TF_LOG_INFO("CompositionRepository::load| Composition loaded: {} v{}.{}", id,
              version.major, version.minor);
  return Result(std::move(composition));
}

Result<Composition> CompositionRepository::LoadLatest(
    const CompositionId& id) {
  const auto version = GetLatestVersion(id);
  if (version.HasError()) {
    TF_LOG_ERROR("CompositionRepository::loadLatest| Error: {}",
                 version.error().message);
...
Result<Version> CompositionRepository::GetLatestVersion(
    const CompositionId& id) {
  auto query =
      box_composition_->query(ObxComposition_::compositionId.equals(id))
          .build();
  auto compositions = query.find();

  if (compositions.empty()) {
    TF_LOG_ERROR(
        "CompositionRepository::getLatestVersion| Composition not found: {}",
        id);
    return Result<Version>(
        Error{ErrorCode::StorageError, "Composition not found"});
  }
...
Error CompositionRepository::deprecate(const CompositionId& id,
  ...
  if (!obx_comp) {
    TF_LOG_ERROR(
        "CompositionRepository::deprecate| Composition not found: {} v{}.{}",
  ...
  // Set state to Deprecated (2)
  obx_comp->state =
      utils::BlockStateToObxStateCode(BlockState::Deprecated);

  if (!box_composition_->put(*obx_comp)) {
    TF_LOG_ERROR(
        "CompositionRepository::deprecate| Failed to save composition");
    return Error{ErrorCode::StorageError, "Failed to save composition"};
  }
